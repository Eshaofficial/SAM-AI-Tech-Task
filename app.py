...
# ============================================================
# IN-MEMORY SQL DATABASE (for the SQL Explorer section)
# ============================================================
@st.cache_resource
def get_sql_connection(_df):
    conn = sqlite3.connect(":memory:", check_same_thread=False)
    _df.to_sql("restaurants", conn, if_exists="replace", index=False)
    return conn

sql_conn = get_sql_connection(df)

PRESET_QUERIES = {
    "Task 2a — Most common cuisine combinations": """
...
LIMIT 10;
""".strip(),
    "Task 2b — Highest-rated cuisine combos (min 10 restaurants)": """
SELECT cuisines, ROUND(AVG(rate), 2) AS avg_rating, COUNT(*) AS num_restaurants
FROM restaurants
...
LIMIT 10;
""".strip(),
    "Task 4a — Top chains by number of outlets": """
SELECT name, COUNT(*) AS num_outlets
FROM restaurants
GROUP BY name
HAVING COUNT(*) > 1
...
LIMIT 10;
""".strip(),
    "Task 4b — Chain vs standalone comparison": """
WITH chain_flag AS (
    SELECT *,
        CASE WHEN name IN (
            SELECT name FROM restaurants GROUP BY name HAVING COUNT(*) > 1
        ) THEN 'Chain' ELSE 'Standalone' END AS restaurant_type
    FROM restaurants
)
...
""".strip(),
    "Task 5a — Top 10 most-voted restaurants": """
SELECT name, location, votes, rate
FROM restaurants
ORDER BY votes DESC
LIMIT 10;
""".strip(),
    "Task 5b — Bottom 10 least-voted restaurants (excl. 0)": """
SELECT name, location, votes, rate
FROM restaurants
WHERE votes > 0
ORDER BY votes ASC
LIMIT 10;
""".strip(),
    "Task 5c — Average rating by vote-count bucket": """
SELECT
    CASE
...
st.caption(
    "Run the same 3 tasks as SQL queries directly against the full dataset "
    "(runs on the complete unfiltered data, independent of the sidebar filters above)."
)

query_mode = st.radio(
    "Query source", ["Preset query", "Write your own SQL"], horizontal=True
)

if query_mode == "Preset query":
    selected_query_name = st.selectbox("Choose a query", list(PRESET_QUERIES.keys()))
    sql_text = PRESET_QUERIES[selected_query_name]
else:
    sql_text = (
        "SELECT name, location, votes, rate\n"
        "FROM restaurants\n"
...
sql_input = st.text_area("SQL query", value=sql_text, height=180)

if st.button("▶️ Run Query"):
    try:
        # Only allow read-only SELECT / WITH queries for safety
        stripped = sql_input.strip().lower()
        if not (stripped.startswith("select") or stripped.startswith("with")):
            st.error("Only SELECT (or WITH ... SELECT) queries are allowed.")
        else:
            result = pd.read_sql_query(sql_input, sql_conn)
